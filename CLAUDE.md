...
The application acts as a frontend that sends user prompts to AI CLIs (`codex` or `claude`), receives structured JSON responses, and presents options in a beautiful color-coded interface.
...
State transitions:
- Input → Running: User presses Enter with non-empty prompt
- Running → Viewing: AI response received (success or error)
- Viewing → Input: User presses Shift+Enter for new prompt
- Viewing → Exit: User presses Enter (copy) or Ctrl+Enter (execute)

### AI CLI Integration

The app supports two AI CLIs with different invocation patterns:
...
3. `/usr/local/share/instassist/options.schema.json` (system install location)
...
- `isShiftEnter()`: Shift+Enter OR Alt+Enter OR Ctrl+J (for adding newlines)
- `isCtrlEnter()`: Ctrl+Enter (for executing commands)

The `handleKeyMsg()` dispatcher routes to mode-specific handlers:
- `handleInputKeys()`: Text entry and submission
- `handleRunningKeys()`: Minimal (only CLI switching)
- `handleViewingKeys()`: Navigation and action selection

### Version Embedding

Version is embedded at build time via ldflags:
```bash
go build -ldflags "-X main.version=$(VERSION)" -o instassist .
```
...
echo "git commands" | ./instassist -output stdout
```
...
- `main.go`: All application code (single-file architecture)
